...
3. Each instance goes through `stopWithForceKill`:
   - `pg.Stop()` (pg_ctl smart mode) with a configurable `stopTimeout` (default 15s).
...
4. Logs shutdown duration on completion.
